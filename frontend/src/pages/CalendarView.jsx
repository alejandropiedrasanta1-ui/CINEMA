...
export default function CalendarView() {
  ...
  return (
    <div className="px-6 py-8 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.1 }} className="glass rounded-3xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/30">
          <motion.button whileHover={{ scale:1.1, x:-2 }} whileTap={{ scale:0.9 }} onClick={prev}
            className="p-2.5 rounded-2xl glass hover:bg-white/50 text-slate-600 transition-colors" data-testid="prev-month-btn"><ChevronLeft size={16} /></motion.button>
          <AnimatePresence mode="wait">
            <motion.h2 key={`${month}-${year}`} initial={{ opacity:0, y:direction*10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:direction*-10 }} transition={{ duration:0.25 }}
              className="text-xl font-black text-slate-900" style={{ fontFamily:'Cabinet Grotesk, sans-serif' }}>
              {tr.months[month]} {year}
            </motion.h2>
          </AnimatePresence>
          <motion.button whileHover={{ scale:1.1, x:2 }} whileTap={{ scale:0.9 }} onClick={next}
            className="p-2.5 rounded-2xl glass hover:bg-white/50 text-slate-600 transition-colors" data-testid="next-month-btn"><ChevronRight size={16} /></motion.button>
        </div>
                ...
                {day && (
                  <>
                    ...
                    <div className="space-y-0.5">
                      {dayEvents.map(ev => (
                        <motion.div key={ev.id} whileHover={{ scale:1.03 }} onClick={() => navigate(`/reservaciones/${ev.id}`)}
                          ...
                          title={`${ev.client_name} — ${ev.event_type}`} data-testid={`calendar-event-${ev.id}`}>
                          {ev.client_name}
                        </motion.div>
                      ))}
                    </div>
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.4 }} className="flex flex-wrap gap-2 mt-5">
